...
const GUVEN = [
  ...
  ["tehlike", "! En zayıf halka: MBU", "Token hızını doğrudan belirleyen şey bant genişliği kullanımıdır (MBU) ve elimde yalnızca İKİ ölçüm noktası var — ikisi de tek cihazdan (DGX Spark). Diğer 39 cihazın değeri tahmindir. Gerçek dağılım geniş: aynı kartta llama.cpp ile vLLM, farklı sürücü ve derleme bayrakları belirgin fark yaratır; yayımlanmış llama.cpp ölçümleri bazı kartlarda buradakinden DÜŞÜK, iyi ayarlanmış vLLM kurulumları YÜKSEK çıkabiliyor. Bu yüzden Donanım panelinde “kendi ölçümünle kalibre et” var: tek bir ölçüm girersen o kartın tüm tahminleri senin gerçeğine oturur. Sayılara güvenmeden önce bunu yapmanı öneririm."],
