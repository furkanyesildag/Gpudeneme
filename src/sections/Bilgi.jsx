...
const VARSAYIMLAR = [
  ...
  ["Decode hızı", "Bellek bant genişliği sınırlı kabul edilir. Adım başına okunan bayt = aktif ağırlık + batch'teki KV cache × 0,55. Gerçekleşen bant genişliği kullanımı (MBU) cihaza göre %57-83."],
...
const GUVEN = [
  ...
  ["uyari", "≈ Mühendislik tahmini", "MBU (%57-83), prefill verimi (%42), KV okuma katsayısı (0,55), MoE ceza üssü, tensor parallelism verimleri ve TFLOPS değerleri. Büyüklük mertebesi ve göreli fark doğrudur, spec-kesin değildir."],
  ["tehlike", "! En zayıf halka: MBU", "Hız modeli yayımlanmış 16 gerçek ölçüme karşı kalibre edildi (hardware-corner: RTX 5090 · dev.to/rosgluk: RTX 4080 16 GB, 6'sı offload'lı · OpenZeka: DGX Spark) — ortalama mutlak sapma %7,3, 16 noktanın 14'ü %15 içinde ve sapmalar iki yönlü. Ama kalibre edilen bu ÜÇ cihaz; kalan 37 cihazın MBU'su bunlardan yığın ve bellek tipine göre ölçeklenmiş tahmindir. Gerçek dağılım geniş: aynı kartta llama.cpp ile vLLM, farklı sürücü ve derleme bayrakları belirgin fark yaratır. Bu yüzden Donanım panelinde “kendi ölçümünle kalibre et” var: tek bir ölçüm girersen o kartın tüm tahminleri senin gerçeğine oturur."],
